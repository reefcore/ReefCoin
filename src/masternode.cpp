...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    // should be at least not earlier than block when 500000 REEF tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 500000 REEF tx -> 1 confirmation
